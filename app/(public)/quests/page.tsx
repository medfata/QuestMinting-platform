...
import { usePaginatedQuests } from '@/hooks/usePaginatedCampaigns';
...
export default function QuestsPage() {
  ...
  const {
    ...
    availableChains,
    loadMore,
  } = usePaginatedQuests(debouncedSearch, selectedChain);
  ...
  return (
    <ThemedContainer theme={homeConfig.theme} applyToDocument as="div">
      ...
      <DynamicHead 
        title={`Quests | ${homeConfig.platform_name}`} 
            ...
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
              <div className="space-y-4">
                ...
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground" style={{ fontFamily: homeConfig.theme.heading_font }}>
                  <span className="bg-gradient-to-r from-secondary via-primary to-secondary bg-clip-text text-transparent bg-[length:200%_auto] animate-gradient-shift">
                    Quests
                  </span>
                </h1>
                <p className="text-muted-foreground text-lg sm:text-xl max-w-xl">
                  Complete tasks and unlock exclusive free mints from top creators
                </p>
              </div>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            {isLoading ? (
              ...
              <>
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {campaigns.map((campaign, index) => (
                    <QuestCard key={campaign.id} campaign={campaign} animationDelay={index < 10 ? index * 50 : 0} />
                  ))}
                </div>
                <InfiniteScroll
                  onLoadMore={loadMore}
              ...
              <div className="rounded-2xl glass p-16 text-center">
                ...
                <button 
                  onClick={handleClearFilters}
                  className="px-6 py-2.5 rounded-xl bg-secondary/10 text-secondary hover:bg-secondary/20 transition-colors font-medium"
                >
                  Clear all filters
                </button>
              </div>
              ...
              <div className="rounded-2xl glass p-16 text-center">
                ...
                <p className="text-lg text-foreground font-medium mb-2">No quests available yet</p>
